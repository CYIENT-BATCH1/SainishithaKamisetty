...
int main()
{
    ...
    case 1:
        reg0=switch_status|((1<<0)|(1<<1));    
          reg1=switch_status&(0x00);          // clear the bits
        reg2=switch_status|((1<<5)|(1<<4));   //fault type1 and 0,1,4,5 bits set to 1 
        for(i=7;i>=0;i--){
            n=(reg0>>i)&1;                    
            printf("%x",n);
        }
        ...
        for(i=7;i>=0;i--){
               ...
               break;
        case 2:
        reg0=switch_status&(0x00);                           //clear the bits
        reg1=switch_status|((1<<7)|(1<<6)|(1<<0)|(1<<1));    //set the bits 0,1,6,7 in Buckle
        reg2=switch_status&(0x00);                           //clear the bits
        n=0x00;
        for(i=7;i>=0;i--){
            n=(reg0>>i)&1;
            printf("%x",n);
        ...
        case 3:
        reg0=switch_status&(0x00);                  //clear the bits
        reg1=switch_status|((1<<4)|(1<<5));         // 4,5,6,7 bits are set condition in unbuckle
        ...
        printf("\n");
               ...
               break;
        case 4:
        reg0=switch_status|(1<<2)|(1<<3);           //1,1,2,2 are set condition in faulty type 2
